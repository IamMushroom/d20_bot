...
Одноразовые ссылки находятся в `web_login_tokens`, активные входы — в `web_sessions`. Исходные
bearer-токены не сохраняются: ключом служит SHA-256-хеш. Просроченные записи не проходят проверку
`expires_at` и лениво удаляются при выпуске новых ссылок.

`AdminAccessService` работает через протокол `WebSessionStore`. Текущая реализация
`SQLiteWebSessionStore` использует общую SQLite, но HTTP-слой не зависит от типа хранилища.
Будущий Redis/Valkey-адаптер должен реализовать тот же контракт выпуска, однократного потребления,
проверки TTL, перечисления и отзыва сессий.
