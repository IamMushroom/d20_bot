...
Outbox хранит структурированные application events, а не готовые Telegram-сообщения. Workflow
публикует только данные операции; connected event consumer валидирует payload и передаёт его
Bot-owned formatter непосредственно перед Telegram API. Фактические схемы событий описаны в
[документации outbox](outbox.md). Presentation formatter недоступен service layer, что закреплено
