...
Для установки тегов персонажей и мастера бот должен быть администратором группы с правом
`can_manage_tags`. Telegram разрешает member tag длиной до 16 символов без emoji и только
для обычных участников. Если Telegram отклонит тег, роль или персонаж всё равно сохранятся
в базе, а бот выведет предупреждение. Для назначения обычного участника мастером
администратор должен ответить командой `/master` на его сообщение.
