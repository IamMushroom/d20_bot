...
В быстром Telegram-меню показываются только `/roll`, `/roll20` и `/duality`.
Остальные команды не удалены: они доступны по прямому вводу и перечислены в `/help`.
...
- `/web_url [https://d20.example]` — показать или задать адрес панели для чата.
...
| `D20_MODE` | нет | `standalone` | `standalone` — только броски; `connected` — броски и пульт Core; `full` — устаревающий монолитный режим. |
| `CORE_URL` | для `connected` | `http://core:8190` в Compose | Внутренний адрес Core API. |
| `CORE_TOKEN` | для `connected` и Core | — | Общий секрет для запросов бота к Core. |
...
Без `D20_MODE` бот работает как standalone-бросалка: не открывает SQLite, не применяет миграции
и не запускает HTTP-сервер. Для запуска достаточно `TG_TOKEN`.
...
Это запускает только лёгкого Telegram-бота без базы и web. Для панели и управления играми
задайте общий секрет и включите профиль `platform`:
...
В этом варианте контейнер `d20` только принимает Telegram-команды, а `core` владеет SQLite,
миграциями и панелью. Переходный `D20_MODE=full` всё ещё поддерживается при прямом запуске,
но для новой установки не рекомендуется.
...
После запуска интерфейс доступен на порту `8080` Raspberry Pi. Порт можно изменить через
`SQLITE_WEB_PORT`. Не публикуйте его в интернет: интерфейс позволяет выполнять запросы и
изменять данные.
...
Панель работает внутри сервиса `core` и не требует JavaScript-фреймворка.
Общий адрес можно указать в окружении:
...
```text
/web_url http://rpi001.local:8190
```

Настройка чата имеет приоритет над `WEB_BASE_URL`.
...
действует 8 часов. Все веб-сессии сбрасываются при перезапуске Core.
Из панели можно назначить или перенести игру, начать сессию с названием и завершить её.
Панель также показывает Telegram ID назначенного мастера и список зарегистрированных персонажей.
Каждое изменение lifecycle публикует соответствующее Telegram-сообщение.
